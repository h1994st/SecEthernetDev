...
struct hashmap *hashmap_new(
  ...
  return hashmap_new_with_allocator(
      (_malloc ? _malloc : malloc), (_realloc ? _realloc : realloc),
      (_free ? _free : free), elsize, cap, seed0, seed1, hash, compare, udata);
}
...
static bool resize(struct hashmap *map, size_t new_cap) {
  struct hashmap *map2 = hashmap_new(
      map->elsize, new_cap, map->seed1, map->seed1, map->hash, map->compare,
      map->udata);
...
void *hashmap_set(struct hashmap *map, void *item) {
  ...
  for (;;) {
    ...
    if (entry->hash == bucket->hash
        && map->compare(bucket_item(entry), bucket_item(bucket), map->udata)
            == 0) {
...
bool hashmap_scan(
    struct hashmap *map, bool (*iter)(const void *item, void *udata),
    void *udata) {
  for (size_t i = 0; i < map->nbuckets; i++) {
    struct bucket *bucket = bucket_at(map, i);
    if (bucket->dib) {
...
static void *xmalloc(size_t size) {
  if (rand_alloc_fail && rand() % rand_alloc_fail_odds == 0) { return NULL; }
...
static void all() {
  ...
  while (
      !(map = hashmap_new(
            sizeof(int), 0, seed, seed, hash_int, compare_ints_udata, NULL))) {}
...
#define bench(name, N, code)                                                   \
  {                                                                            \
    {                                                                          \
      ...
      printf(                                                                  \
          "%d ops in %.3f secs, %.0f ns/op, %.0f op/sec", N, elapsed_secs,     \
...
static void benchmarks() {
  ...
  map = hashmap_new(
      sizeof(int), N, seed, seed, hash_int, compare_ints_udata, NULL);
  bench("set (cap)", N, {
    int *v = hashmap_set(map, &vals[i]);
    assert(!v);
  }) shuffle(vals, N, sizeof(int));
