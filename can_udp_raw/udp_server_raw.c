...
//    printf("\n\n");
  ...
  struct can_frame *can = (struct can_frame *) payload;
  ssize_t can_frame_len = sizeof(struct can_frame) - CAN_MAX_DLEN
      + can->can_dlc;  // including CAN frame header
  memcpy(&con, payload + can_frame_len, sizeof(con));
