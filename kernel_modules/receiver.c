...
void mitm_skb_rht_fini(void) { rhashtable_destroy(&mitm_skb_hash_tbl); }
...
handle_proof_packets(struct mitm *mitm, struct sk_buff *skb) {
  int ret;
  ...
  if (!skb_entry) {
    netdev_err(
        mitm->dev,
        "drop the proof packet, as no corresponding packet exists!\n");
    return MITM_DROP;
  }
  ...
  ret = crypto_shash_digest(
      desc, skb_mac_header(old_skb), old_skb->tail - old_skb->mac_header, data);
  ...
  // remove the entry
  netdev_info(
      mitm->dev, "remove skb_entry=%p from the hash table\n", skb_entry);
  rhashtable_remove_fast(
      &mitm_skb_hash_tbl, &skb_entry->rhnode, mitm_skb_rht_params);
