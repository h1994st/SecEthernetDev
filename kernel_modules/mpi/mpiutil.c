...
MPI mpi_const(enum gcry_mpi_constants no) {
  if ((int) no < 0 || no > MPI_NUMBER_OF_CONSTANTS)
    pr_err("MPI: invalid mpi_const selector %d\n", no);
  if (!constants[no]) pr_err("MPI: MPI subsystem not initialized\n");
  return constants[no];
}
...
MPI mpi_set_ui(MPI w, unsigned long u) {
  if (!w) w = mpi_alloc(1);
  /* FIXME: If U is 0 we have no need to resize and thus possible allocating the
   * limbs.
   */
...
void mpi_swap_cond(MPI a, MPI b, unsigned long swap) {
  ...
  if (a->alloced > b->alloced) nlimbs = b->alloced;
  else
    nlimbs = a->alloced;
